...
## 1. システム概要
ユーザーが指定したGitHubリポジトリを自動で解析し、そのコードの重要な概念、アルゴリズム、アーキテクチャ、言語仕様に関する **「穴埋めクイズ」** と **「逆引きドキュメント」** を自動生成する学習支援プラットフォームである。
...
* 未ログインでの公開リポジトリ取得は未認証で行うため、60req/h の制限がかかる。これは「2.4 冗長な処理の排除」のキャッシュで吸収する。同一コミットの解析結果は利用者をまたいで共有するため、同じリポジトリへの2回目以降はGitHub APIをほとんど消費しない。
* ログインして本人のPATを登録すれば、そのユーザーの解析は5,000req/hで実行される。
